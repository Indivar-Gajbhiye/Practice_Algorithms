...
public class LongestSubStringWithoutRepititon {
	...
	static String longestUniqueSubstringWRepeatingCharacter(String str)
	{
		...
		for (i = 1; i < len; i++) {
			 ...
			 * * non repeating character string, simply increase the length of
			...
			/*
			 * If the current character is present in currently considered non
			 * repeating character string, then update non repeating character
			 * string to start from the next character of previousIndex.
			 */
			else {
				/*
				 * Before updating non repeating character, check for maximum
				 * length
				 */
